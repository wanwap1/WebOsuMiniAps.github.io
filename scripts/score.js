    ...
    function addPlayHistory(summary) {
        ...
        if (window.db && window.telegramUser && window.telegramUser.id > 0) {
            ...
            const scoreData = {
                ...
                username: window.telegramUser.username || window.telegramUser.first_name,
                score: parseInt(summary.score), // Преобразуем строку в число для рейтинга
                accuracy: parseFloat(summary.acc.replace('%', '')),
