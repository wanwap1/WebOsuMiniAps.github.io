// Ждем, пока DOM (структура страницы) будет готова,
// прежде чем запускать код игры
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.Telegram && window.Telegram.WebApp) {
    ...
    // 1. Сообщаем Telegram, что приложение готово к отображению
    tg.ready();

    // 2. Растягиваем приложение на весь экран (ВАЖНО для ПК)
    //    Именно эта команда делает окно широким
    tg.expand();

    // 3. Скрываем стандартную кнопку "Назад" Telegram,
    //    так как в игре она не нужна
    tg.BackButton.hide();
  ...
  // ----- НАЧАЛО ТВОЕГО ОРИГИНАЛЬНОГО КОДА INITGAME.JS -----
